...
const Navbar = () => {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-40 transition-all duration-500 py-5 ${isScrolled
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-24 flex items-center justify-between relative z-50">
          ...
          >
            <div className="flex flex-col gap-1.5 justify-center items-center w-5 h-5">
              <span className={`w-5 h-[1.5px] bg-white rounded-full transition-all duration-300 origin-center ${isOpen ? 'rotate-45 translate-y-[4.5px]' : ''
                }`} />
              <span className={`w-5 h-[1.5px] bg-white rounded-full transition-all duration-300 ${isOpen ? 'opacity-0' : 'opacity-100'
                }`} />
              <span className={`w-5 h-[1.5px] bg-white rounded-full transition-all duration-300 origin-center ${isOpen ? '-rotate-45 -translate-y-[4.5px]' : ''
                }`} />
            </div>
      ...
      {/* Mobile Drawer Menu */}
      <div className={`fixed inset-0 z-30 md:hidden bg-[#08080a]/98 backdrop-blur-lg flex flex-col justify-center items-center transition-all duration-500 ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
        }`}>
